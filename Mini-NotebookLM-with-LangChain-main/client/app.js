...
function decorateAnswer(div, data) {
  if (data.interrupt?.type === "source_selection") {
    showSourceSelection(data.interrupt.sources);
    return;
  }
...
function showSourceSelection(sources) {
  ...
  div.innerHTML = `
    ...
    <button class="select-sources-btn">
      Continue
    </button>
  `;
  ...
  div.querySelector("button").onclick = () => {
    const selected = [...div.querySelectorAll("input:checked")]
      .map(cb => sources[cb.value]);

    resumeAgent(selected);
  };

  $("messages").appendChild(div);
}

async function resumeAgent(selectedSources) {
  console.log("RESUME CLICKED", selectedSources);
  ...
  const data = await api.send("POST", "/api/chat/resume", {
    thread_id: state.threadId,
