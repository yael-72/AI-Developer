...
def resume_chat(thread_id: str, decision) -> ChatResponse:
    result = chat.resume(thread_id, decision)
...
def add_source(name: str | None, content: str) -> SourceInfo:
    ...
    source = store.add(name=name, content=content)

    print("SOURCE SAVED:")
    print("Name:", source.name)
    print("Length:", len(source.content))
    print(source.content[:2000])

    return _to_info(source)
